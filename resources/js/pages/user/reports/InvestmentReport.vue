<template>
    <div>
        ...
        <div id="appCapsule">
            <div class="section">
                <div class="row mt-2">
                    <div v-if="investment.length > 0" class="card p-2">
                            ...
                            <div class="row">
                                <div class="col-sm-12 col-md-6">
                                    <div class="dataTables_length" id="example_length"><label>Show <select
                                                v-model="entries" @change="getInvestment(1)" name="example_length"
                                ...
                                <div class="col-sm-12 col-md-6">
                                    <div id="example_filter" class="dataTables_filter"><label>Search:<input
                                                ...
                                                placeholder="" @change="getInvestment(1)"
                                    ...
                                    <table id="example"
                                        ...
                                        <thead class="text-primary">
                                            ...
                                            <tr>
                                                ...
                                                <th class="text-primary">Countdown</th>
                                                <th class="text-primary">Status</th>
                                            </tr>
                                        </thead>
                                        <tbody v-if="loading == false">

                                            <tr v-for="data in investment">
                                                <td data-label="Activation">{{ data.date }}</td>
                                                <td data-label="Plan">{{ data.name }}</td>
                                                <td data-label="Capital">${{ $root.numeral(data.amount) }}</td>
                                                <td class="text-success" data-label="Interest">+
                                                    ${{ $root.normalNumeral(data.roi) }}</td>

                                                <td data-label="Countdown" class="text-success">
                                                    {{ data.expiration ? getDate(data.expiration) : ' ' }}</td>
                                                <td class="pb-0">
                                                    <div v-if="data.active && data.expiration && data.loop"
                                    ...
                                    <div class="dataTables_paginate paging_simple_numbers" id="example_paginate">
                                        <ul class="pagination">
                                            ...
                                            <li v-if="paging" v-for="num in paging.total_pages" :class="{
                                                ...
                                                <a href="#" @click.prevent="getInvestment(num)" aria-controls="example"
                    ...
                    <div v-else-if="investment.length == 0 && loading == false" class="card p-2">
                        <div id="example_wrapper" class="dataTables_wrapper dt-bootstrap5 no-footer">
                            <div class="row">
                                <div class="col-sm-12">
...
export default {
    ...
    mounted() {
        this.$root.dashboard_header_page_title = "Investment History"
        setTimeout(() => { window.scrollTo(0, 0); }, 1000);
        this.getInvestment(this.current_page)
    },
    created() {
    ...
    methods: {
        getInvestment(page) {
            ...
            let form = new Form()
            form.get("auth/packageusers?user_id=" + this.$auth.user().id + '&page=' + page + searchQuery + "&pageSize=" +
                this.entries)
                .then(response => {
                    this.$root.loader('hide')
                    this.investment = response.data.data.item
                    this.totalAsset = this.investment.sum('amount')
                    this.loading = false
